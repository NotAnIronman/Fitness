...
const ACHIEVEMENTS = [
  ...
  // ---- Workouts ----
  { id: 'first_workout',      name: 'First Rep',            desc: 'Log your first workout exercise.',                       category: 'Workouts', points: 10, check: ctx => ctx.totalExercisesLogged >= 1 },
  { id: 'workouts_10',        name: 'Getting Into It',      desc: 'Log 10 exercises total.',                                category: 'Workouts', points: 20,check: ctx => ctx.totalExercisesLogged >= 10, progress: ctx => ({ current: Math.min(ctx.totalExercisesLogged, 10), target: 10 }) },
  { id: 'workouts_50',        name: 'Regular',              desc: 'Log 50 exercises total.',                                category: 'Workouts', points: 50,check: ctx => ctx.totalExercisesLogged >= 50, progress: ctx => ({ current: Math.min(ctx.totalExercisesLogged, 50), target: 50 }) },
  { id: 'workouts_100',       name: 'Dedicated',            desc: 'Log 100 exercises total.',                               category: 'Workouts', points: 100,check: ctx => ctx.totalExercisesLogged >= 100, progress: ctx => ({ current: Math.min(ctx.totalExercisesLogged, 100), target: 100 }) },
  { id: 'workout_streak_7',   name: 'Weekly Grind',         desc: 'Log a workout on 7 days in a row.',                      category: 'Workouts', points: 30,check: ctx => ctx.workoutStreak >= 7, progress: ctx => ({ current: Math.min(ctx.workoutStreak, 7), target: 7 }) },
  { id: 'all_body_parts',     name: 'Well Rounded',         desc: 'Log an exercise from every major body part.',            category: 'Workouts', points: 40,check: ctx => ctx.bodyPartsTouched >= 7, progress: ctx => ({ current: ctx.bodyPartsTouched, target: 7 }) },
  { id: 'ramping_master',     name: 'Ramping Up',           desc: 'Log an exercise with a different weight per set.',       category: 'Workouts', points: 15,check: ctx => ctx.usedPerSetWeights },
