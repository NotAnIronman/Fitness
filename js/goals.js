  ...
  return `
    <div class="card">
      ...
      <div class="grid grid-3" style="margin-bottom:12px;">
        ...
        <div class="stat"><div class="stat-label">Daily calorie target</div><div class="stat-value accent">${suggestedIntake ? calculationTip(`${Math.round(suggestedIntake)}<span class="unit">kcal/day</span>`, 'How the daily target is estimated', `Maintenance estimate ${Math.round(effTdee)} kcal/day plus the first-pass energy adjustment needed for ${rateDisplay}. The adjustment uses the familiar 3,500 kcal-per-pound approximation; real weight change is dynamic, so trend data should replace this estimate over time.`, ['mifflin','energyPlanner','gradualLoss'], 'Published BMR + Forge planning model') : 'Not provided'}</div>${targetLow ? `<div class="hint">${calculationTip(`Model uncertainty: ${Math.round(targetLow)}-${Math.round(targetHigh)}`, 'What this range means', `Start near ${Math.round(suggestedIntake)} kcal/day. The lower number is not automatically the better target. The range shows how uncertainty in maintenance carries into this calculation; use your 2–4 week weight trend to calibrate the target.`, ['energyPlanner'], 'Forge uncertainty allowance')}</div>` : ''}</div>
      </div>
      ${suggestedIntake ? `<div class="section-note"><strong>What to eat:</strong> Start near ${Math.round(suggestedIntake)} kcal/day. The range is uncertainty around that target, not an instruction to choose its lowest value.</div>` : ''}
